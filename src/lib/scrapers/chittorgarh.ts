...
// ─────────────────────────────────────────────────────────
// Lightweight HTTP fetch helper (no browser needed)
// Works on Vercel serverless, local dev, everywhere.
// ─────────────────────────────────────────────────────────
...
export async function scrapeChittorgarhIPOs() {
  console.log('🚀 Starting Chittorgarh All-Cap IPO Scraper (fetch-based, no browser)...');
  ...
  for (const target of targets) {
    ...
    try {
      const html = await fetchHtml(target.url, 30000);
      const $ = cheerio.load(html);

      const trs = $('table tbody tr');
      console.log(`Found ${trs.length} rows in ${target.name} list.`);

      // Process top 30 freshest IPOs per category (increased from 20)
      ...
      for (let i = 0; i < processCount; i++) {
        ...
        if (openDate && today >= openDate) {
          if (closeDate && today > closeDate) {
            stage = listingDate && today >= listingDate ? 4 : 3;
          } else {
            stage = 2;
          }
        }
        ...
        // ────────────────────────────────────────────────────
        // Detail page scrape — pure fetch, no browser
        // ────────────────────────────────────────────────────
        if (fullDetailUrl) {
          try {
            const detailHtml = await fetchHtml(fullDetailUrl, 15000);
            const $d = cheerio.load(detailHtml);
            // Plain text version for regex-based extraction
            ...
            // ── 3. Lot size & Min investment (4 strategies) ─
            let lotSize = '';
            let minInvestment: number | null = null;

            // A: table with "Lot Size" label
            ...
            // B: Application / Lot table with Retail (Min) row
            $d('table').each((_, tableEl) => {
            ...
            // C: body text regex using first word of company name
            if (!companyDescription) {
              const shortName = companyName.split(' ')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
              const rx = new RegExp(`About\\s+${shortName}[\\w\\s,\\.\\-&]*?\\n([\\s\\S]{60,500}?)(?:\\n\\n|\\n[A-Z])`, 'i');
              const bm = bodyText.match(rx);
              if (bm) companyDescription = bm[1].trim();
            }

            // ── 5. Objects of Issue (6 variants) ────────────
            let objectsText = '';
            ...
            if (!objectsText) {
              $d('h2, h3, h4').each((_, el) => {
                if (objectsText) return false;
                const heading = $d(el).text().trim().toLowerCase();
                if (OBJ_KEYWORDS.some(kw => heading.includes(kw.split(' ')[0]))) {
                  const text = $d(el).next('p, ul, ol, div').text().trim();
            ...
            if (stage >= 2) {
              ...
              try {
                ...
                // Anchor investors — collect rows first, then insert (no async inside .each)
